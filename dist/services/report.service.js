...
exports.generateUserDailyReport = generateUserDailyReport;
exports.generateAllUserReports = generateAllUserReports;
...
async function generateDailyReport(date) {
    ...
    console.log(`[report.service] 全局日报生成完成 — date: ${formatDate(targetDate)}, 频道数: ${reports.length}, 总条目: ${totalItemCount}`);
 ...
 * @param date 报告日期
 */
async function generateUserDailyReport(userId, date) {
    const targetDate = new Date(Date.UTC(date.getFullYear(), date.getMonth(), date.getDate()));
    // 查询用户的 ACTIVE 订阅
    ...
    console.log(`[report.service] 用户日报生成完成 — userId: ${userId}, date: ${formatDate(targetDate)}, 频道数: ${dedupedReports.length}`);
    return { content, result: { shouldPush: true } };
}
/**
 ...
 */
async function generateAllUserReports(date) {
    const targetDate = new Date(Date.UTC(date.getFullYear(), date.getMonth(), date.getDate()));
    // 查询有 ACTIVE 订阅的用户
    const activeUserIds = await prisma_1.prisma.subscription
        .findMany({
        where: { status: 'ACTIVE' },
        select: { userId: true },
        distinct: ['userId'],
    ...
    const results = await Promise.all(activeUserIds.map(async (userId) => {
 ...
 * 构建报告 Markdown 内容
 */
function buildReportContent(date, channels, reports) {
    const dateStr = formatDate(date);
    const sections = [`# 情报日报 ${dateStr}`, ''];
    const channelMap = new Map(channels.map((c) => [c.id, c.name]));
    for (const report of reports) {
        if (!report.processedContent || report.itemCount === 0)
            continue;
