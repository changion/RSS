...
router.post('/', auth_middleware_1.authenticateJWT, async (req, res) => {
    try {
        const { title, description, contentType, sourceHint, customPrompt } = req.body;
        ...
        const channelRequest = await prisma_1.prisma.channelRequest.create({
            data: {
...
router.get('/me', auth_middleware_1.authenticateJWT, async (req, res) => {
    try {
        const requests = await prisma_1.prisma.channelRequest.findMany({
...
router.patch('/admin/:id/approve', auth_middleware_1.authenticateJWT, (0, auth_middleware_1.requireRole)(['ADMIN']), async (req, res) => {
    try {
        const { id } = req.params;
        const { adminNote, channelId } = req.body;
        const channelRequest = await prisma_1.prisma.channelRequest.findUnique({
            where: { id },
        ...
        const updated = await prisma_1.prisma.channelRequest.update({
            where: { id },
            data: {
                status: 'APPROVED',
        ...
        if (channelRequest.user.feishuWebhook) {
            ...
            (0, feishu_service_1.sendFeishuNotification)(channelRequest.user.feishuWebhook, {
                title: '频道需求申请已批准',
...
//# sourceMappingURL=channel-request.routes.js.map