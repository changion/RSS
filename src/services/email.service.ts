...
import { recordPushLog } from './feishu.service';
 ...
 * 构建完整邮件 HTML 模板
 */
function buildEmailHtml(report: string, date: Date): string {
 ...
 */
export async function pushAllUserEmailReports(
  date: Date,
  userReports: { userId: string; content: string }[],
): Promise<void> {
  ...
  for (const { userId, content } of userReports) {
    ...
    if (!emailAddress || !content) {
      if (!emailAddress) {
        await recordPushLog(targetDate, 'EMAIL', 'SKIPPED', 'no email configured', userId);
      }
      continue;
    }
    ...
    try {
      ...
      await recordPushLog(targetDate, 'EMAIL', 'SENT', undefined, userId);
    } catch (err) {
      const errorMsg = err instanceof Error ? err.message : String(err);
      console.error(`[email.service] 邮件推送失败 — userId: ${userId}:`, errorMsg);
      await recordPushLog(targetDate, 'EMAIL', 'FAILED', errorMsg, userId);
    }
  }
}
